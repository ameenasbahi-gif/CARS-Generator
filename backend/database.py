...
DB_PATH = os.environ.get("DB_PATH", "passages.db")
...
def get_passage_by_id(passage_id: int) -> Optional[dict]:
    conn = get_conn()
    try:
        p = conn.execute("SELECT * FROM passages WHERE id=?", (passage_id,)).fetchone()
        if not p:
            return None
        questions = conn.execute(
            "SELECT * FROM questions WHERE passage_id=? ORDER BY seq", (passage_id,)
        ).fetchall()
        result = dict(p)
        result["questions"] = [
            {
...
def get_random_passage(category: str = "all") -> Optional[dict]:
    ...
    try:
        if category == "all":
            row = conn.execute("SELECT id FROM passages ORDER BY RANDOM() LIMIT 1").fetchone()
        else:
            row = conn.execute(
